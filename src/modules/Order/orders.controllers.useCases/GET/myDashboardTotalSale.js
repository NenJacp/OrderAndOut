...
/**
 * @description Controlador para obtener el total de ventas en el dashboard
...
const handle = async (req, res) => {
    ...
    try {
        const totalSale = await orderService.getTotalSaleByDateRangeAndRestaurantId(start, end, restaurantId);
        res.status(200).json({ data: { totalSale }});
    } catch (error) {
        console.error('Error en dashboard total sale controller:', error);
        res.status(500).json({
            success: false,
