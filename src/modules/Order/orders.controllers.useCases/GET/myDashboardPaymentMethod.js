...
/**
 * @description Controlador para obtener estadísticas de pago en el dashboard
...
const handle = async (req, res) => {
    ...
    try {
        const paymentMethods = await orderService.getOrdersByPaymentMethodByDateRangeAndRestaurantId(start, end, restaurantId);
        res.status(200).json({ data: paymentMethods });
    } catch (error) {
        console.error('Error en dashboard payment method controller:', error);
        res.status(500).json({
            success: false,
            message: 'Error al obtener estadísticas de pago',
